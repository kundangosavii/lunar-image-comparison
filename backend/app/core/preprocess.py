...
def Resolution_Resampling(
    ...
    The sensor names are passed explicitly by the user/frontend.
    """

    source_sensor = str(source_sensor).upper()
    reference_sensor = str(reference_sensor).upper()

    if source_sensor not in SENSOR_RESOLUTION:
        raise ValueError(f"Unknown source sensor: {source_sensor}")

    if reference_sensor not in SENSOR_RESOLUTION:
        raise ValueError(f"Unknown reference sensor: {reference_sensor}")

    source_resolution = SENSOR_RESOLUTION[source_sensor]
    reference_resolution = SENSOR_RESOLUTION[reference_sensor]

    target_resolution = max(source_resolution, reference_resolution)
    ...
    if source_resolution != target_resolution:
        ...
        print(f"Source: already at {target_resolution} m/px")

    reference_h, reference_w = reference_image.shape[:2]

    if reference_resolution != target_resolution:
        scale = reference_resolution / target_resolution
        new_w = max(1, int(round(reference_w * scale)))
        new_h = max(1, int(round(reference_h * scale)))
        ...
        print(f"Reference: already at {target_resolution} m/px")

    return source_image, reference_image

def process_all(source, reference, source_sensor, reference_sensor):
    source_original = source.copy()
    reference_original = reference.copy()

    source_normalized = normalize(source_original)
    reference_normalized = normalize(reference_original)
